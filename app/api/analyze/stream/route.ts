...
function buildFastVisionMessages({
  ...
  return [
    {
      role: "system",
      content: `你是一个学习题目解析助手。请直接识别图片中的题目并解答，用 ${outputLanguageText(language)} 输出自然语言 Markdown。
请输出：
1. 识别到的题目
2. 最终答案
3. 分步骤解析
...
要求：
- 直接解题，不要描述图片边框、截图界面或无关 UI。
- 不要说“根据图片中可见信息”。
- 不要说“图片内容复杂”。
- 如果看不清题目，请明确说“题目不清晰，无法可靠识别”。
- 复杂题可以完整分步骤推导，不要为了缩短回答牺牲解析完整度。
- 公式用 LaTeX。`
    },
    {
      role: "user",
...
async function createCachedResponse({
  ...
  const status = mode === "analysis" || quizResult ? "completed" : "explanation_done";
  ...
  try {
    ...
    if (cached) {
      ...
      const stream = new ReadableStream({
        start(controller) {
          controller.enqueue(encoder.encode(sseEncode("meta", cachedResponse.payload)));
          controller.enqueue(encoder.encode(sseEncode("done", cachedResponse.payload)));
          controller.close();
        }
      });
