...
所有数学公式必须使用 KaTeX 可渲染的标准 LaTeX：行内公式写成 $...$，独立公式写成 $$...$$，分式写成 $\\frac{a}{b}$，根号写成 $\\sqrt{x}$，幂次写成 $x^2$。禁止在 \frac 后插入多余的美元符号，禁止多余的 $，禁止代码块包公式。坐标、交点、区间必须写成 $\\left( ... \\right)$，变量和物理符号也要写成数学形式，例如 $x$、$y$、$v$、$F$、$m$、$a$、$k$。输出语言：${outputLanguage === "en" ? "English" : "中文"}。
JSON 格式：
${QUIZ_JSON_SHAPE}`
    },
    {
