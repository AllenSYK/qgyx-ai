...
如果是题目解析，必须跟随用户或系统选择的输出语言，不要中英混用：
- 中文结构固定为：题目、过程、答案。
- English structure must be: Question, Process, Answer.
除题目原文必须保留的符号和专有名词外，中文模式全部用中文讲解，English mode must use English for all explanatory text.
过程要简洁但能看懂，通常写 3-5 个关键步骤，说明关键公式或代入原因。
