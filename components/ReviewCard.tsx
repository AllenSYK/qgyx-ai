...
export default function ReviewCard({ originalAnalysisText, wrongQuestions }: ReviewCardProps) {
  ...
  async function requestReview() {
    ...
    try {
      ...
      setReview((data?.data || data) as ReviewResult);
    } catch {
    ...
    <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-card sm:p-7">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <button
          ...
          disabled={loading}
          className="inline-flex items-center justify-center gap-2 rounded-2xl bg-amber-500 px-5 py-3 text-sm font-semibold text-white transition duration-200 ease-out hover:bg-amber-600 active:scale-[0.97] active:opacity-75 disabled:cursor-not-allowed disabled:bg-amber-300 disabled:opacity-75"
        >
      ...
      {review ? (
        <div className="mt-6 space-y-5 transition duration-[250ms] ease-out">
          <div className="rounded-3xl border border-amber-100 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
            ...
            <MarkdownRenderer text={review.weaknessSummary} className="ml-1" />
          </div>
